...
- (id)initWithFrame:(CGRect)frame titleName:(NSString *)title {
    ...
    if (self) {
        ...
        //add subView
        self.arrow = [[UIImageView alloc] initWithImage:[UIImage imageNamed: MyBundleImage(@"pulldown.png")]];
...
- (void)updateTitleState:(BOOL)isSelected {
    if (isSelected) {
        ...
        self.arrow.image = [UIImage imageNamed:MyBundleImage(@"pullup")];
    } else{
        self.titleLabel.textColor = [UIColor blackColor];
        self.arrow.image = [UIImage imageNamed:MyBundleImage(@"pulldown")];
    }
}
